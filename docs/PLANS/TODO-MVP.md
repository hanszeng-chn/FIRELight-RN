...
- [ ] 实现月份选择器 (MonthPicker)
  - [ ] 年月下拉选择 UI ("2026年 1月 v")
  - [ ] 月收入/支出汇总显示
- [ ] 实现按日期分组的条目列表 (SectionList)
  - [ ] 日期 Header (日期 + 当日收支统计)
  - [ ] 条目 Item (图标 + 分类名 + 金额)

### 3.2 交互逻辑
- [ ] 默认加载最新有数据的月份 (无数据则显示当前月)
- [ ] 月份切换时刷新数据
- [ ] 实现空状态 UI ("本月暂无账目，快记一笔吧")

### 3.3 FAB 悬浮按钮
- [ ] 实现右下角悬浮按钮 (+ 记一笔)
- [ ] 点击跳转至记账页面 (`/app/add-transaction`)
...
### 4.1 页面结构
- [ ] 实现页面 Header (返回按钮 + 支出/收入 Tab)
- [ ] 实现分类网格 (Swipeable Page / 4列布局)
  - [ ] 分类图标 + 名称
  - [ ] 列表末尾添加 "设置" 入口 (跳转分类管理)

### 4.2 数字键盘模块 (Bottom Sheet / Overlay)
- [ ] 实现弹出式输入模块
- [ ] 金额显示区域 (大字号)
- [ ] 备注输入框
- [ ] 数字键盘布局
  ...
  - [ ] 回退键 (Backspace)
  - [ ] 完成键 (OK)
  - [ ] 日期键 (显示 "今天" 或 具体日期 "MM-DD")

### 4.3 业务逻辑
- [ ] 点击分类图标 -> 弹出键盘模块
- [ ] 日期键交互 -> 弹出系统日期选择器 -> 更新键文案
- [ ] 金额校验 (> 0)
- [ ] 提交 -> 保存数据 -> 返回首页并刷新
...
### 5.2 新增/编辑弹窗 (Bottom Sheet)
- [ ] 名称输入框
- [ ] 图标预览 (自动生成: 首字大写, 无手动选择)
- [ ] 确认按钮
- [ ] 校验逻辑 (不为空、不重复)
...
- [ ] 停用/删除逻辑 (点击 ⛔️):
  - [ ] 物理删除: 无历史数据的自定义分类
  - [ ] 软删除 (停用): 系统分类 或 有数据的自定义分类
- [ ] 恢复逻辑: 移回启用列表底部
- [ ] 自动清理 (Auto GC): 进入页面时物理删除已停用且无数据的自定义分类
...
- [ ] 配置路由结构 (Expo Router)
  - [ ] `/app/(tabs)/index` (首页)
  - [ ] `/app/add-transaction` (记一笔)
  - [ ] `/app/category-management` (分类管理)
